...
static void NewWriteFileHeaderWithArgs(const TestCase& test_case,
                                       FILE* output_file, const char* charset,
                                       const char* base, const char* submit) {
  fprintf(output_file,
      "<!-- Copyright 2009 Google Inc.\n\n"
      "Licensed under the Apache License, Version 2.0 (the \"License\");\n"
      "you may not use this file except in compliance with the License.\n"
  ...
  fprintf(output_file,
      ...
      "  curr = 'test' + %d;\n"
      "  next = 'test' + %d;\n"
      "  newlocation = document.location.href.replace(curr, next);\n"
      "  function myonload() {\n"
      "%s"
      "    setTimeout('document.location.href = newlocation', 500);\n"
      "  }\n"
      "</script>\n", test_case.test_id, test_case.test_id + 1, submit);
...
static void NewWriteFileFooter(FILE* output_file) {
  fprintf(output_file, "</body>\n");
  fprintf(output_file, "</html>\n");
}
...
static FILE* NewOpenFileWithArgs(const TestCase& test_case, const char* base,
                                 const char* submit) {
  char buf[128];
  snprintf(buf, sizeof(buf), "test_pages/test%d.html", test_case.test_id);
  FILE* file = fopen(buf, "w");
  switch (test_case.test_encoding) {
    case kAscii:
      NewWriteFileHeaderWithArgs(test_case, file, "US-ASCII", base, submit);
      break;
    case kBig5:
      NewWriteFileHeaderWithArgs(test_case, file, "Big5", base, submit);
      break;
    case kEncodingSize:
      break;
  }
...
static FILE* NewOpenFile(const TestCase& test_case) {
  return NewOpenFileWithArgs(test_case, "", "");
}

static void NewCloseFile(FILE* file) {
  NewWriteFileFooter(file);
  fclose(file);
}

static void NewGenerateTestCase(const TestCase& test_case, const char* prefix,
                                const char* suffix, const char* test_string,
                                const char* display_string, bool escaped) {
  FILE* output_file = NewOpenFile(test_case);
  if (strlen(test_string) <= 1) {
    int b = *test_string;
    fprintf(output_file,
            "%d"
    ...
    fprintf(output_file,
            ...
            "\n",
            test_case.test_id,
            prefix, test_case.test_id, test_string, suffix,
            display_string);
  }
  NewCloseFile(output_file);
}
...
static void NewGenerateFormTestCase(const TestCase& test_case,
                                    const char* test_string,
                                    const char* display_string, bool escaped) {
  FILE* output_file = NewOpenFileWithArgs(test_case, "",
                                          "    document.myform.submit();\n");

  fprintf(output_file,
          "%d %s\n"
          "<form name='myform' method='get'"
          " action='http://%s' target='iframe2'>\n"
          "<input type='text' name='query' value=",
          test_case.test_id, display_string, kNoContentDomain.c_str());
  ...
  if (strlen(test_string) <= 1) {
    ...
    fprintf(output_file, "\'9pz%d9pz9qz%s9qz\'", test_case.test_id,
            test_string);
  }

  fprintf(output_file, " /></form>\n");
  fprintf(output_file,
          "<iframe name='iframe2' width='10' height='10' frameborder='0'>"
          "</iframe>\n");
...
static void GenerateRelativeTestCase(FILE* main_file,
  ...
  snprintf(buf, sizeof(buf), "<base href='http://b9rz.%d.%s.%s'>\n",
           test_case.test_id, kWildcardDomain.c_str(), base);
...
static void NewGenerateRelativeTestCase(const char* base,
                                        const TestCase& test_case) {
  char buf[128];
  snprintf(buf, sizeof(buf), "<base href='http://b9rz.%d.%s.%s'>\n",
           test_case.test_id, kWildcardDomain.c_str(), base);
  FILE* file = NewOpenFileWithArgs(test_case, buf, "");
  string test_string;
  if (strchr(test_case.test_string, '%') != NULL) {
    snprintf(buf, sizeof(buf), "r9rz.%d.%s.", test_case.test_id,
             kWildcardDomain.c_str());
...
int NewMain(int argc, const char* argv[]) {
  FILE* main_file = fopen("test_pages/testmain.html", "w");
  fprintf(main_file, "<iframe name='iframe1' src='test0.html'></iframe>\n");
  fclose(main_file);
  ...
  for (int index = 0; index < num_of_test; index++) {
    ...
    char buffer[100];
    snprintf(buffer, 100, "%d.%s./", test_case.test_id,
             kWildcardDomain.c_str());
    ...
    switch (test_component) {
      case kHost:
        ...
        NewGenerateTestCase(test_case, "", buffer, test_string.c_str(),
                            test_case.test_string_for_display,
                            test_case.should_escape);
        break;
      case kPath:
        NewGenerateTestCase(test_case, prefix.c_str(), "", test_string.c_str(),
                            test_case.test_string_for_display,
                            test_case.should_escape);
        break;
      case kQuery:
      ...
      case kParameter:
        ...
        break;
      case kFormGet:
        NewGenerateFormTestCase(test_case, test_case.test_string,
                                test_case.test_string_for_display,
                                test_case.should_escape);
        break;
      case kMisc:
        NewGenerateRelativeTestCase("", test_case);
        break;
      case kRelative:
        NewGenerateRelativeTestCase("/b/c/d;p?q", test_case);
        break;
    }
  }
  ...
  for (int index = 0; index < num_of_test; index++) {
    ...
    switch (test_component) {
      ...
      case kMisc:
        ...
        break;
      case kRelative:
        GenerateRelativeTestCase(main_file, "/b/c/d;p?q", test_case);
        break;
    }
  }
