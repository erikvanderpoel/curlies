...
static void WriteResultTableHeading(FILE *output_file, const bool isAscii, const vector<string>& platform_browser_under_test)
{
    fprintf(output_file, "<tr><th>ID</th><th>Test</th>");
    if(isAscii){
        fprintf(output_file, "<th>Char</th>");
    }
    for(vector<string>::const_iterator it = platform_browser_under_test.begin();it != platform_browser_under_test.end();it++){
...
static void WriteFileHeader(FILE* output_file,
  ...
  fprintf(output_file,
    ...
    WriteResultTableHeading(output_file, isAscii, platform_browser_under_test);
}
